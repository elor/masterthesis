...
Mit der Kupfer-PVD wurde ein zweiter Metall-PVD-Prozess auf das Parsivald-Modell angepasst und unter der Zielsetzung simuliert, die Anwendbarkeit verschiedener Potentialparametrisierungen zu prüfen.
...
\begin{table}
  ...
  \begin{tabularx}{\textwidth}{|lXc|}
    ...
    Cu\_u3.eam                  & Oberflächen und Bulksysteme verschiedener Legierungen                        & \cite{foiles_embedded-atom-method_1986}  \\
...
Die meisten anderen Parametersätze unterschätzen die Bindungslänge nur leicht, mit Ausnahme der mit LAMMPS verbreiteten Dateien (smf7, u3, u6), welche ideale strukturelle Eigenschaften produzieren.
...
\todo{Satz nochmal anschauen}Mit den untersuchten Parametrisierungen wird der Schmelzpunkt stark überschätzt, mit Ausnahme von cu\_ag\_ymwu.eam.alloy, mit deren Nutzung der Schmelzpunkt um 300K in Simulationen unterschätzt wird (Abbildung~\ref{fig:copperthermo}).
...
Wie Tabelle~\ref{tab:copperpots} zu entnehmen ist, wurden die verfügbaren Parametersätze nicht für thermodynamische Untersuchungen angepasst und unterstützen diese somit nicht, doch werden solche extremen Temperaturen in den durchgeführten Simulationen ohnehin nicht erreicht.
...
\begin{figure}
  ...
  \begin{subfigure}[t]{\subfigwidth}
    ...
    \subcaption{Dichteabhängigkeit von der Temperatur}
  \end{subfigure}
  \hfill
  \begin{subfigure}[t]{\subfigwidth}
    \includegraphics[width=\textwidth]{Cu_smf7_meltingpoint}
    \subcaption{Dichteabhängigkeit bei smf7 in Abh. von $t_\text{relax}$}
  \end{subfigure}
  ...
  \todoline{Legenden (Parametersätze und Temperatur) + linke Grafik neu (Legende überlagert sich). Die Grafiken sind klein.}
\end{figure}
...
\todoline{So formulieren, dass sekundäre Sputterprozese (nicht direkt nennen) eigentlich gut sind}
Auch bei Kupfer-PVD zeigt die Simulation epitaktisches Wachstum glatter Schichten.
Zu Beginn der Abscheidung ergeben sich extreme Abbruchraten von \SI{25}{\percent}, die durch Herausschlagen einzelner Oberflächenatome bei Ankunft des gesputterten Atomes verursacht werden.
...
Mit einem nahezu konstanten RMS-Wert um \SI{1}{\angstrom} sind die gewachsenen Schichten durchgehend glatt, mit Ausnahme einzelner Simulationsläufe.

Eines der untersuchten Cu\_u3.eam-Systeme bildet beispielsweise ab Schritt 70 eine \SI{2.6}{\nano\meter} breite und \SI{3.0}{\nano\meter} tiefe kraterförmige Vertiefung aus, die schließlich zu einem \SI{1}{\nano\meter} großen Hohlraum abgeschlossen wird (Abbildung~\ref{fig:coppercrater}).
Die Auswirkungen dieses Kraters auf die Rauheit sind in Abbildung~\ref{fig:copperparsivald-c}  klar als stetige Zunahme, gefolgt von einer schlagartigen Abnahme in Schritt 96, und in den Abbruchraten (Abbildung~\ref{fig:copperparsivald-a}) als Anstieg zwischen den Schritten 60 und 90 erkennbar, haben aber keinen Einfluss auf die Wachstumsrate (Abbildung~\ref{fig:copperparsivald-b}).
...
Die \todo{Wort! -> Workerdichten}\todo{Workerdichte vorstellen}Bedeckungsdichten für Systeme unterschiedlicher Größe wurden bereits in Tabelle~\ref{tab:goldscalability} für Gold-Abscheidungen aufgelistet und zeigen ähnliche Werte für kleine Systeme, können für Substrate auf der Mikrometer-Skala mangels verfügbarer Rechenleistung jedoch nicht aufrecht erhalten werden.

\todoline{Joerg: Da du es für Gold so intensiv diskutiert hast, stellt sich hier auch die Frage nach dem Vergleich mit Experiment. Eventuell könnte man sich damit rausreden, dass man das am Beispiel von Gold so ausführlich gemacht hat und sich bei Kupfer eher auf den Vergleich verfügbarer MD-Parametrisierungen beschränkt}
