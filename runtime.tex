...
Für kleine Simulationsräume lässt sich die maximale Anzahl von Ereignissen $p_\text{max,1}-1$ aus der \todo{überlappungsfrei?}\todo{Ergänzen, was die mittlere Packung von zufällig verteilten Quadraten auf der Oberfläche ist}dichtesten Packung von meist quadratischen MD-Boxen der Breite $w_\text{MD}$, Tiefe $d_\text{MD}$ und Fläche $A_\text{MD} = w_\text{MD} \cdot d_\text{MD}$ im Simulationsraum ($w_\text{sim}$, $d_\text{sim}$, $A_\text{sim}$) abschätzen:
\begin{equation}
...
\begin{figure}[p]
  ...
  \todoline{grüne Fläche für den effizienten Bereich weglassen}
...
Für große Räume mit $w_\text{sim} > w_\text{eff}$ ist die Gesamt-Laufzeit außerdem von der MD-Laufzeit unabhängig\todo{Begründen und Grenzen ansprechen}.
...
Die Grenze zwischen diesen Bereichen lässt sich als effiziente Raumgröße $w_\text{eff}$ ermitteln, welche den größten Simulationsraum beschriebt, der in minimaler Laufzeit simuliert werden kann.
