...
Ein Vergleich mit den in Abschnitt~\ref{runtime} vorgestellten Laufzeitwerten wird im Folgenden anhand von Gold-PVD durchgeführt, für welche Abscheidungen auf unterschiedlich großen quadratischen Substraten mit Parsivald \todo{Hardware erwähnen. wie viele Kerne insgesamt? Heterogener Cluster?}simuliert wurde.
...
Ein Vergleich der Laufzeiten mit Werten für $T_p$ aus Gleichung~\ref{eq:runtime} zeigt ebenfalls gute Übereinstimmungen, wobei der Ausreißer bei $w_\text{sim} = \SI{1}{\micro\meter}$ ebenfalls erkennbar ist.%legitequationreference
...
\begin{figure}[H]
  ...
  \end{subfigure}
  \caption{Parallelisierungseigenschaften von Parsivald im Vergleich zu den Modellrechnungen (Abschnitt~\ref{runtime})}
  \label{fig:goldscala}
  \todoline{``Modell'' statt ``Analytisch'', ``Parsivald (Max)'' und Parsivald (``Mittelw.'')}
  \todoline{(c) Analytisch nicht mit ``Maximum''-Markern versehen}
\end{figure}
