...
parser.add_argument('files', metavar='data.txt', type=str, default='Si_rdfdata.txt', nargs='+', help='data files, containing columns of numerical data')
parser.add_argument('-x', '--xcolumn', dest='xcolumn', type=int, default=0, help='column for x-axis data')
parser.add_argument('-y', '--ycolumn', dest='ycolumn', type=int, default=2, help='column for y-axis data')
parser.add_argument('-f', '--to-file', dest='tofile', action='store_true', default=True, help='print to pdf and png files instead of interactive output')
...
parser.add_argument('--ylabel', dest='ylabel', type=str, default="bond length", help='y label (LaTeX compatible)')
parser.add_argument('--bar', dest='bar', action='store_true', default=True, help='produce a bar plot instead of lines')
...
for index, filename in enumerate(args.files):
    ...
    if args.bar:
        ...
        plt.plot(data[args.xcolumn], data[args.ycolumn], '-', color=colors(index), label=tex_compatible_filename)
...
if args.tofile:
    print('saving to pdf')
    plt.savefig('Si_bondlengths.pdf')
    plt.close()
    call(["pdf2png.sh", 'Si_bondlengths.pdf'])
else:
    plt.show()
