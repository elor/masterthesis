...
\todoline{Limitierungen der Workerdichte (gleichzeitigen Worker)}
\todoline{Beschreibung der Workerdichte}
\todoline{Welche Größen bei Nanodevices von Interesse?}
\todoline{warum nur 38.4 Worker?}
\todoline{Wie viele Worker theoretisch?}
\todoline{Welche Limitierung durch Hostprozess?}
\todoline{Welche Limitierung durch maximale Dichte}
\todoline{Skalierung... womit?}
\todoline{``Rekord'' vermeiden}
\todoline{realistische Werte}
\todoline{Bedeckungsgrade - definieren}
\todoline{Was wirkt eigentlich limitierend, und was sind sinnvolle Größen?}
