...
Als Resultat entstand eine Software namens \textit{Parsivald}, mit der die atomistische Simulation von Gasphasenabscheidungen auf der Größenordnung kompletter Nano-Bauelemente \todo{nicht wirklich möglich. Aufteilen, was jetzt schon geht und was anvisiert wird}bis zu \SI{1x1}{\micro\meter} ermöglicht wird, was bis zu \num{1e9} Atomen entspricht.
Dies wird durch Nutzung effizienter Datenstrukturen und einem Host-Worker-Schema der Parallelisierung ermöglicht.

\todo{Die hier vorliegenden klaren Zusammenfassungen in den jeweiligen Abschnitten ergänzen}
...
\todoline{ggf. Streichen: das keine Bildung von Nanopartikeln zeigt, wie sie im Experiment durch AFM-Untersuchungen zu beobachten sind.
...
Weiterhin ergaben auch Abscheidungssimulationen von Gold-PVD auf strukturierten Substraten \todo{eigentlich ja hauptsächlich, oder?}überwiegend epitaktisches Wachstum.
In einigen Fällen \todo{Wo?}formten sich zudem Nanoporen an groben Unebenheiten der Struktur, welche die Oberflächenrauheit dominierten, sich jedoch im Laufe der Simulation langsam schlossen und so Hohlräume innerhalb der Schicht bildeten.
...
\todoline{Stärken und Ergebnisse hervor heben, nicht Schwächen und Probleme}
...
In Abscheidungssimulationen von mehrlagigen Schichten per Kupfer-Nickel-PVD sind perfekte Übereinstimmungen mit molekulardynamischen Simulationen in Rauheit und Struktur zu beobachten.
Im direkten Vergleich konnte dabei gezeigt werden, dass der Parsivald-Ansatz gegenüber der reinen MD eine enorme Beschleunigung von \todo{wie viel?}xx bringt.
\todo{weglassen: wodurch sich für dieses System eine Abwesenheit von Finite-Size-Effekten aufgrund des Parsivald-Modelles zeigt.}
Auch für dünne Lagen mit einer Dicke von nur \SI{1}{\nano\meter} sind klare Lagengrenzen vorhanden.
Deren Ausprägung nimmt mit der Relaxationszeit und der \todo{Teilchenenergie?}Auftreffenergie zu, was in guter Übereinstimmung mit experimentellen \todo{Befunden?}Befunden ist\todo{Ref!}.
\todoline{zu detailliert für eine Zusammenfassung: wofür allerdings die Relaxationstemperatur sowie die Auftreffenergie des gesputterten Teilchens leicht erhöht werden musste.
Der selbe Einfluss der Teilchenenergie auf die Lagenqualität wurde jedoch auch im Experiment beobachtet.
...
\todo{weglassen: Mit fortschreitender Simulationszeit ist eine Schließung der Poren wie bei den kraterförmigen Vertiefungen der Kupfer-PVD-Simulationen zu erwarten, wodurch die Rauheit begrenzt wäre.}
...
Bei der Simulation von Oberflächen-Reaktionen von Wasser mit Aluminiumoxid mit reaktiven Kraftfeldern kann jedoch eine \todo{relativieren}perfekte Hydroxylierung beobachtet werden, die gute Übereinstimmung der Hydroxyl-Bedeckung mit experimentellen Werten zeigt.
Allerdings konnte der zweite Precursor der \ce{Al2O3}-ALD, Trimethylaluminium\todo{Überarbeiten: Beim Precursor stabil, nur mit der Parametrisierung in der Simulation nicht}, mangels stabiler \ce{Al-C}-Bindungen nicht von den untersuchten ReaxFF-Parametrisierungen beschrieben werden.
...
\section{Ausblick}
\todoline{viel zu detailliert}
\todoline{Unterscheidung zwischen Parsivald und möglichen Anwendungen}
\todoline{Beschreibung ACCELERATE oder Kurzbeschreibung des Promotionsvorhabens}
...
\todo{weglassen: Die Simulation von verspannten Multilagen-Systemen, bei denen durch unterschiedliche Gitterkonstanten ein Gitterversatz zu beobachten ist, führt zu der Frage, inwiefern Parsivald den untersuchten Systemen eine kristalline Struktur aufzwingt.}
...
Diese könnten neben molekulardynamischen Potentialen \todo{Wort}theoretisch auch auf präzisere Methoden wie Elektronenstrukturrechnungen zurück greifen.
